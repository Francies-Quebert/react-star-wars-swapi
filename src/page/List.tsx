...
function List() {
    ...
    const [type, setType] = useState<List<ListTypes>>(searchParams.get('type') || 'people')
    const { data, error , isFetching, } = useGetListQuery({ page, type }, { refetchOnFocus: false })

    useEffect(() => {
        setType(searchParams.get('type') ?? 'people')
    ...
    return (
        <div className='filter__page'>
            <div className='px-1 filter__page_control'>
                ...
                {isFetching && <div className='fetch'><span className='capitalize'>{type}</span> are being enlightened..</div>}
            </div>
            <Table
                column={column}
